...
for N = [1000, 2000, 3000, 4000] % sample size
    for alpha = [0.05, 0.1]    % risk level
        %% choose the method
        IP = 1; CVa = 1; DC = 1; pDC = 1; pDC1 = 1; pDC2 = 1; Bi = 1; SC = 1; repeat_num = 5;
        ...
        for iter = 1:repeat_num
            ...
            if iid == 1
                ...
                for i = 1:m
                    train_samples(:, :, i) = normrnd(0, 1, N, n); % N x n x m
                end
            else
                train_samples = gensample(N, n, m);
            end
            %% parameter setting
            solver = 'gurobi'; maxitime = 1800; tol = 1e-6; U = 100;

            fprintf('***************** num of repeat: %d ***************** \n', iter);
            %% solve MIP by Gurobi
            if IP == 1
                opts = struct('solver', solver, 'maxitime', maxitime); upper = 1e4;
                tic; [x_MIP,time_MIP] = MIP(train_samples, U, upper, alpha, opts); ttime_MIP = toc;
            ...
            if CVa == 1
                ...
                tic; [x0,time_CVaR] = CVaR(train_samples, U, alpha, opts); ttime_CVaR = toc;
            ...
            if Bi == 1
                opts = struct('solver', solver, 'maxitime', maxitime);
                tic; [x_Bi, iter_Bi] = BiCVaR(train_samples, U, alpha, opts); ttime_Bi = toc;
            ...
            if DC == 1
                ...
                tic; [x_DC, time_DC, iter_DC] = DCA(train_samples, U, alpha, opts); ttime_DC = toc;
            ...
            if pDC == 1
                ...
                tic; [x_PD, time_PD, iter_PD] = pDCA(train_samples, U, alpha, beta, opts); ttime_PD = toc;
            ...
            if pDC1 == 1
                ...
                tic; [x_PD1, time_PD1, iter_PD1] = pDCA(train_samples, U, alpha, beta, opts); ttime_PD1 = toc;
            ...
            if pDC2 == 1
                ...
                tic; [x_PD2, time_PD2, iter_PD2] = pDCA(train_samples, U, alpha, beta, opts); ttime_PD2 = toc;
            ...
            if SC == 1
                solver = 'gurobi'; epsilon = 1e-1; init = 1; %% init = 1: initialization by CVaR
                if init == 1
                    opts = struct('solver', solver, 'maxitime', maxitime, 'x0', x0, 'tol', tol);
                else
                    opts = struct('solver', solver, 'maxitime', maxitime, 'tol', tol);
                end
                tic; [x_SC, time_SC, iter_SC] = SCA(train_samples, U, alpha, epsilon, opts); ttime_SC = toc;
